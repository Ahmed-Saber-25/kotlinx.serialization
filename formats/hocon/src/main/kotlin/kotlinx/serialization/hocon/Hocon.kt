...
public sealed class Hocon(
    ...
    private inner class ConfigReader(val conf: Config) : ConfigConverter<String>() {
        ...
        override fun <T> decodeSerializableValue(deserializer: DeserializationStrategy<T>): T {
            ...
            val config: Config = if (currentTagOrNull != null) conf.getConfig(currentTag) else conf;

            val reader = ConfigReader(config)
            val discriminator = classDiscriminator
            val type = reader.decodeTaggedString(discriminator)
...
public class HoconBuilder internal constructor(hocon: Hocon) {
     ...
     * This is an option for legacy HOCON format and should not be generally used.
     * `false` by default.
     */
